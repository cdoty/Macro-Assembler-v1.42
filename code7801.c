...
static CPUVar CPU7801;

static SReg *SRegs;

static ASSUMERec ASSUME7801s[] =
{
...
static void DecodePUSH(Word Code)
{
  ShortInt HReg;
...
static void InitFields(void)
{
  ...
  AddInstTable(InstTable, "PUSH", 0x48, DecodePUSH);
  AddInstTable(InstTable, "POP", 0x48, DecodePOP);
  ...
  AddInstTable(InstTable, "SKIT", 0x00, DecodeSKIT_SKNIT);
  AddInstTable(InstTable, "SKNIT", 0x10, DecodeSKIT_SKNIT);
  ...
  AddFixed("JEA"  , 0x4828); AddFixed("CALB" , 0x63);
  AddFixed("SOFTI", 0x0072); AddFixed("RET"  , 0x0008);
  AddFixed("RETS" , 0x0018); AddFixed("RETI" , 0x0062);
  AddFixed("NOP"  , 0x0000); AddFixed("EI"   , 0x4820);
  AddFixed("DI"   , 0x4824); AddFixed("HLT"  , 0x01);
  ...
  AddAbs("CALL", 0x0044); AddAbs("JMP" , 0x0054);
  ...
  AddReg2("RLL" , 0x4830); AddReg2("RLR" , 0x4831);
  AddReg2("SLL" , 0x4833); AddReg2("SLR" , 0x4834);
  AddReg2("SLLC", 0x4835); AddReg2("SLRC", 0x4836);

  AddWork("DCRW", 0x30); AddWork("INRW", 0x20);
  AddWork("LDAW", 0x28); AddWork("STAW", 0x38);
...
static void MakeCode_7801(void)
{
  CodeLen = 0;
  DontPrint = False;
...
static void InitCode_7801(void)
{
  WorkArea = 0x100;
}

static Boolean IsDef_7801(void)
{
  return False;
}

static void SwitchFrom_7801(void)
{
  DeinitFields();
}

static void SwitchTo_7801(void)
{
  ...
  pASSUMERecs = ASSUME7801s;
  ASSUMERecCnt = sizeof(ASSUME7801s) / sizeof(*ASSUME7801s);

  MakeCode = MakeCode_7801; IsDef = IsDef_7801;
  SwitchFrom = SwitchFrom_7801;
  InitFields();
}

void code7801_init(void)
{
  CPU7801 = AddCPU("7801" , SwitchTo_7801);

  AddInitPassProc(InitCode_7801);
}
